...
python3 SKILL_DIR/scripts/prompt_harness.py \
  ...
  --var SKILL_DIR='$SKILL_DIR' \
  --var REFS_DIR='$SKILL_DIR/references' \
  --inject-file PPTX_EXPORT_SAFETY=SKILL_DIR/references/design-runtime/pptx-export-safety.md \
  --inject-file PPTX_HTML_COMPAT=SKILL_DIR/references/design-runtime/pptx-html-compat.md \
...
```bash
# 0. SpeechSynth prompt + subagent
python3 SKILL_DIR/scripts/prompt_harness.py \
  --template SKILL_DIR/references/prompts/tpl-speech-synth-phase1.md \
  --var REQUIREMENTS_PATH=OUTPUT_DIR/requirements-interview.txt \
  --var BRIEF_PATH=CURRENT_BRIEF_PATH \
  --var OUTLINE_PATH=OUTPUT_DIR/outline.txt \
  --var STYLE_PATH=OUTPUT_DIR/style.json \
  --var PLANNING_DIR=OUTPUT_DIR/planning \
  --var SLIDES_DIR=OUTPUT_DIR/slides \
  --var TOTAL_PAGES=TOTAL \
  --var SPEECH_OUTPUT=OUTPUT_DIR/speech-script.json \
  --inject-file PLAYBOOK=SKILL_DIR/references/playbooks/speech-phase1-playbook.md \
  --output OUTPUT_DIR/runtime/prompt-speech-phase1.md

python3 SKILL_DIR/scripts/prompt_harness.py \
  --template SKILL_DIR/references/prompts/tpl-speech-synth-phase2.md \
  --var SPEECH_OUTPUT=OUTPUT_DIR/speech-script.json \
  --var BRIEF_PATH=CURRENT_BRIEF_PATH \
  --var OUTLINE_PATH=OUTPUT_DIR/outline.txt \
  --var SLIDES_DIR=OUTPUT_DIR/slides \
  --var TOTAL_PAGES=TOTAL \
  --inject-file PLAYBOOK=SKILL_DIR/references/playbooks/speech-phase2-playbook.md \
  --output OUTPUT_DIR/runtime/prompt-speech-phase2.md

python3 SKILL_DIR/scripts/prompt_harness.py \
  --template SKILL_DIR/references/prompts/tpl-speech-synth-orchestrator.md \
  --var PHASE1_PROMPT_PATH=OUTPUT_DIR/runtime/prompt-speech-phase1.md \
  --var PHASE2_PROMPT_PATH=OUTPUT_DIR/runtime/prompt-speech-phase2.md \
  --var SPEECH_OUTPUT=OUTPUT_DIR/speech-script.json \
  --output OUTPUT_DIR/runtime/prompt-speech-orchestrator.md

# 创建 SpeechSynth subagent：
#   SUBAGENT_NAME = SpeechSynth
#   MODEL         = SUBAGENT_MODEL
#   PROMPT_PATH   = OUTPUT_DIR/runtime/prompt-speech-orchestrator.md

# 1. Gate + 派生可读演讲稿
python3 SKILL_DIR/scripts/contract_validator.py speech-script OUTPUT_DIR/speech-script.json --expected-pages TOTAL
python3 SKILL_DIR/scripts/speech_script_formatter.py OUTPUT_DIR/speech-script.json -o OUTPUT_DIR/speech-script.md
...
python3 SKILL_DIR/scripts/png2pptx.py OUTPUT_DIR/png -o OUTPUT_DIR/presentation-png.pptx --speech-script OUTPUT_DIR/speech-script.json

# 4. SVG 管线（与 PNG 并行）
python3 SKILL_DIR/scripts/html2svg.py OUTPUT_DIR/slides -o OUTPUT_DIR/svg
python3 SKILL_DIR/scripts/svg2pptx.py OUTPUT_DIR/svg -o OUTPUT_DIR/presentation-svg.pptx --html-dir OUTPUT_DIR/slides --speech-script OUTPUT_DIR/speech-script.json
...
> `run_id`、`generated_at`、`artifacts`（含五个路径）为 validator 强制校验字段；`summary` 和 `pages` 建议填写。
