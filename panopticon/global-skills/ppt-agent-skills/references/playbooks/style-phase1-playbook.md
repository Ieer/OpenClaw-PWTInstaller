# Style Phase 1 Playbook -- 风格合同的定义与输出

## 目标

基于用户需求、大纲结构和 runtime 风格规则，输出一份可被 planning/html 稳定消费的 `style.json` 全局风格合同，而不是任何单页的 HTML 代码。

---

## 阶段执行流程

### Phase 1: 提炼风格约束

1. 读取 `requirements-interview.txt`
2. 抽取高优先级信号：品牌色、品牌禁区、受众、正式度、语言、配图策略。绝不允许在不知情的情况下选错受众（如给技术向听众做可爱风）。
3. 读取大纲 `outline.txt`
4. 判断整套 deck 的节奏类型：稳态推进 / 波浪起伏 / 发布会式冲刺 / 培训式均匀展开，以决定你的变化策略基调。

### Phase 2: 创造核心风格调色盘（拒绝机械查表）

不要做生搬硬套的“填表员”！**用户的需求（风格、受众、场景）拥有绝对的第一优先级！**
在过去，我们过于依赖几个有限的“预置风格基底”（如 blue_white, dark_tech 等），导致无论受众和需求多么独特，最终配出来的颜色全都大同小异。这是不可接受的！

**决策你的核心风格取向：**
1. **深度提炼用户采访需求**：仔细品味 `requirements-interview.txt` 中 `style`、`brand` 和受众画像的描述。
2. **原创调色盘**：大胆抛弃那仅有的几个老套预置模版！凭借你的高级审美，从零开始直接写出高度定制化、最契合当前任务的 12 个关键 `css_variables`（主背景色、卡片梯度、4个 accent 强调色）。
3. **只在极度迷茫时，才参考预置文件**。

如果用户要求“神秘东方”、“孟菲斯几何”、“末日废土”、“二次元高亮”或者本例中特有的“技术社区感”，**请毫无顾忌地直接输出原创的十六进制色彩搭配**。用户的 `style` 需求不只是参考，它是**最高设计法典**，你配置的 `accent` 和背景色必须与其100%同频。

### Phase 3: 生成 style.json 合同

你必须输出一份严格遵守下列字段要求的 JSON 合同文件：

*   `style_id` / `style_name`
*   `mood_keywords`：**（强制：必须提供 3-5 个关键词的数组）**
*   `design_soul`：描述整套 deck 的设计目标，**绝对不可以**写成某一页的成品描述或构图指导。
*   `variation_strategy`：必须同时说明“哪些元素允许变”和“哪些元素锁死不动”。不能写成逐页执行指令。
*   `decoration_dna.signature_move`：必须有，且为非空字符串。
*   `decoration_dna.forbidden`：**（强制：必须提供 2-5 个元素的数组）**
*   `decoration_dna.recommended_combos`：**（强制：必须提供 2-4 个元素的数组）**
*   `font_family`

#### decoration_dna 具体度红线

`decoration_dna` 不是情绪形容词列表，而是后续 PageAgent 可以直接执行的视觉动作库：

- `signature_move` 必须写成可落地动作，例如“12° 斜切荧光编号条 + 右下角半透明雷达网格”，不要写成“简洁现代 / 高级科技 / 专业商务”。
- `recommended_combos` 每一项都必须包含“形状/纹理/位置/色彩角色”中的至少两类信息，例如“左侧 2px accent_2 竖线 + 顶部 18% 低透明噪点渐变”。
- `forbidden` 要禁止具体元素或处理方式，例如“禁止满屏蓝色渐变背景”“禁止无数据来源的 3D 图标”，不要只写“不要花哨”。
- 风格语言必须绑定本次主题、受众或品牌禁区；如果换到任意 deck 仍成立，就说明太泛化，需要重写。

#### css_variables 键命名与数量规范（强制红线）

这 12 个变量是基石，必须定义并且键名**不能更改一个字母**：

```json
{
  "bg_primary": "#...",
  "bg_secondary": "#...",
  "card_bg_from": "#...",
  "card_bg_to": "#...",
  "card_border": "#...",
  "card_radius": "...px",
  "text_primary": "#...",
  "text_secondary": "#...",
  "accent_1": "#...",
  "accent_2": "#...",
  "accent_3": "#...",
  "accent_4": "#...",
  "css_snippets": {
    "example_class": "font-weight: 700;"
  }
}
```

- key 必须使用下划线（无 `--` 前缀），对应校验合同要求。
- 必须严格保留这 12 个基础变量名，禁止改名。如需自定义增加可以增加，但这 12 个不可少。
- `css_snippets` 必须是对象 (Object)，格式为 `"键名": "值"`，**绝对不能是数组 (Array)！** 确有必要时可用它固化跨页重复的局部样式结构（如阴影），但绝对不能包含能驱动整页骨架布局的 CSS。
