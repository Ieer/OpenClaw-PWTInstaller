...
当 planning 出现 `layout_hint=one-pager-grid`、`visual_weight>=8`、`negative_space_target=low` 或用户明确要求一页简报时，允许比普通内容页更丰富，但必须用结构组织密度：

- 使用 12 栏或近似 12 栏的 grid 思维：顶部结论带、KPI 短卡、主洞察、行动/风险 rail、source strip 各自有稳定区域。
- 通过“面积、色彩权重、字号层级、编号流向”建立阅读顺序，不要把 6 个卡片做成同权重表格墙。
- 正文默认仍保持 `13.5-15px`；只有 caption / source / 更新时间可降到 `10-11px`，且必须高对比。
- KPI / metric 小卡优先使用短标签 + 大数字 + 1 行解释，不写长段正文。
- 行动建议和风险项优先做成 2-3 条 action rail，不与主洞察抢锚点。
- source strip 放在主内容区底部内侧，和 `footer.slide-footer` 保持至少 `20px` 间距。
- 如果一页需要 7 个以上主模块，必须合并：把相近模块折叠成列表、metric row 或来源条，不得继续扩卡片。

单页简报必须让观众按以下顺序读完：`结论 → 关键数字 → 主洞察/证据 → 影响 → 建议动作/风险`。如果视觉顺序不清，宁可减少内容也不能放任成“信息拼贴”。
