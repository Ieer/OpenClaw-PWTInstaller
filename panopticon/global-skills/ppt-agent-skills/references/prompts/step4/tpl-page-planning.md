...
2. 深度读取 `{{REQUIREMENTS_PATH}}`，将其中的【受众画像】、【目标动作】、【期望页数】和【版面心智】作为单页选型和内容设计的最高约束（例如：对底层技术受众放大图表卡片，对合作方主打对比及成果锚点）。若发现“1页简报 / one-pager / 高密度但可读 / 容量极大”信号，必须启用 Playbook 的高密度安全模式，而不是退回普通低密度内容页。
