...
**PPTX export guardrails**: Step 4 now injects export-focused safe-area and HTML compatibility rules, reducing cases where a slide looks fine in the browser but degrades in PPTX delivery.
...
P5 Speech Script + Preview Generation + Dual PPTX Export
```
...
All outputs are written to `ppt-output/runs/<RUN_ID>/`, including a browser-previewable HTML gallery, `speech-script.md`, and both PPTX formats.
