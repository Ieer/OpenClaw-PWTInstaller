...
Usage:
    python thumbnail.py input.pptx [output_prefix] [--cols N] [--width N] [--dpi N] [--outline-placeholders]
...
Examples:
    ...
    python thumbnail.py high-res.pptx --width 800 --dpi 300
    # Creates higher resolution thumbnails
...
# Constants
THUMBNAIL_WIDTH = 600  # Fixed thumbnail width in pixels
CONVERSION_DPI = 200  # DPI for PDF to image conversion
...
def main():
    ...
    parser.add_argument(
        "--dpi",
        type=int,
        default=CONVERSION_DPI,
        help=f"DPI for intermediate rendering (default: {CONVERSION_DPI})",
    )
    parser.add_argument(
        "--no-labels",
        action="store_true",
        help="Do not draw slide numbers above thumbnails",
    )
    ...
    if not input_path.exists() or input_path.suffix.lower() not in [".pptx", ".pdf"]:
        print(f"Error: Invalid file: {args.input} (must be .pptx or .pdf)")
        sys.exit(1)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if args.outline_placeholders:
                if input_path.suffix.lower() == ".pdf":
                    print("Warning: --outline-placeholders is not supported for PDF files. Skipping.")
                else:
            ...
            slide_images = convert_to_images(input_path, Path(temp_dir), args.dpi)
            if not slide_images:
...
def convert_to_images(input_path, temp_dir, dpi):
    """Convert PowerPoint or PDF to images."""
    input_path = Path(input_path).resolve()
    
    if input_path.suffix.lower() == ".pdf":
        print(f"Converting PDF to images at {dpi} DPI...")
        result = subprocess.run(
            ["pdftoppm", "-jpeg", "-r", str(dpi), str(input_path), str(temp_dir / "slide")],
            capture_output=True,
            text=True,
        )
...
def create_grid(
    ...
    # Calculate vertical spacing per cell
    if no_labels:
        cell_content_height = height
        thumbnail_offset_y = 0
    else:
        cell_content_height = height + font_size + label_padding * 2
        thumbnail_offset_y = font_size + label_padding * 2

    # Calculate grid size
    rows = (len(image_paths) + cols - 1) // cols
    grid_w = cols * width + (cols + 1) * GRID_PADDING
    grid_h = rows * cell_content_height + (rows + 1) * GRID_PADDING
    ...
    for i, img_path in enumerate(image_paths):
        ...
        y_base = row * cell_content_height + (row + 1) * GRID_PADDING
        ...
        # Add thumbnail below label (if exists)
        y_thumbnail = y_base + thumbnail_offset_y
