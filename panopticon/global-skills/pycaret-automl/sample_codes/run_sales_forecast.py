...
def main() -> None:
    ...
    sort_metric = "MASE"
    best_model = experiment.compare_models(sort=sort_metric, verbose=False)
    comparison_table = experiment.pull()

    holdout_predictions = experiment.predict_model(best_model, verbose=False)
    holdout_metrics = experiment.pull()
    ...
    holdout_metrics.to_csv(output_dir / "holdout_metrics.csv", index=True)
