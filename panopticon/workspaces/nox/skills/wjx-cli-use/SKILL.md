...
当用户需要创建问卷、查询答卷、导出数据、分析问卷结果、管理问卷星联系人/部门/子账号，或明确提到 wjx / 问卷星 / survey / questionnaire 时使用。

## Quick Reference

先确认意图与 API Key 状态；创建问卷默认走 `create-by-json`；执行后用自然语言报告结果，不向用户暴露后台命令，除非用户明确要求。

## Output

- 问卷创建、查询、导出、分析或配置检查结果。
- 关键链接、问卷 ID、导出文件路径或错误归因。
- 需要用户提供 API Key 或人工确认时，输出下一步而不继续重试。

## Safety Rules

- 不记录 API Key 明文。
- API Key 缺失或失效时停止业务动作，先请求用户提供或重新配置。
- 删除、覆盖、批量同步、外部提交类动作必须先确认。
