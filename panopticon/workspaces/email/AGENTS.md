...
After bootstrap, use `sources/inner-map-skill-router/SKILL.md` as the default first-pass router when the task is still mixed, under-defined, emotionally loaded, communication-heavy, calibration-heavy, or mainly about distilling reusable communication knowledge. Do not force that router for clear mailbox triage, explicit drafting-only tasks with settled boundaries, narrow execution-only tasks, or heartbeat I/O.
...
- problem convergence from mixed inbox signals, follow-up pressure, and communication tradeoffs
- communication framing for sensitive replies, escalation notes, or boundary-setting
- 7/30/90 day calibration for follow-up discipline, inbox habits, or response systems
- distilling recurring email patterns, promises, and communication lessons into reusable knowledge
...
If the task becomes a formal send/reply/forward decision, reply strategy judgement, commitment boundary call, or follow-up sequencing risk decision, switch to the knowledge-eval default below instead of staying in inner-map routing.

#### Routing Examples

- `Inner-map / dialog-management`: "收件箱里有催办、投诉、内部同步和未决事项，我不知道先处理哪封。" 先收敛问题，再给最小下一步。
- `Inner-map / communication-coach`: "我要回这封邮件，但既不能太硬，也不能给出错误承诺。" 先处理对象、边界、语气和风险。
- `Inner-map / excellence-calibration`: "我想把接下来一个月的邮件跟进和响应习惯拉回正轨。" 先做阶段校准，再给 7/30/90 天动作。
- `Inner-map / self-management`: "把这次关于邮件承诺边界和回复策略的结论整理进知识体系。" 先提炼高价值信息，再判断是否提升到 inner-map knowledge。
- `Knowledge-eval`: "这封投诉邮件现在该不该回，还是先等更多信息？" 这已经是 formal recommendation，直接走 knowledge-eval。
- `Knowledge-eval`: "对方要我在邮件里承诺交期，这个能不能写？" 这属于承诺风险判断，不停留在 inner-map。
- `Hybrid`: "我现在情绪很乱，但最终要决定这封邮件是直接回、升级，还是先等。" 先用 inner-map 收敛，再切到 knowledge-eval 做正式判断。

Quick rule:

- 如果用户主要在说“我很乱、怎么说、怎么梳理、怎么复盘、怎么沉淀”，先走 inner-map。
- 如果用户主要在说“该不该回、该不该转、能不能承诺、跟进顺序怎么排”，先走 knowledge-eval。
- 如果同时存在两类信号，先收敛，后评估，不要反过来。

### Knowledge Evaluation Default

For email's common workflows, use `skills/knowledge-eval/` before giving formal recommendations when the task is about:

- send / reply / forward decisions
- reply strategy or tone judgement
- commitment boundary or wording risk
- follow-up sequencing and external communication risk
...
3. Answer in email style: recommended draft direction, risks, follow-up, and whether Review is required

If `summary.status` is `no_hit` or `weak_hit`, mark it as limited evidence and do not recommend outbound action directly.
