...
- 默认不把同一任务同时写入 `artifacts/`/`sources/` 和 inner-map knowledge；只有明确需要长期个人知识沉淀时，才做提升归档。
- 路由快规则：如果用户主要在说“我很乱、怎么说、怎么复盘、怎么沉淀”，先走 inner-map；如果主要在说“取消还是保留、退款还是更改、哪个方案更稳”，先走 knowledge-eval。
- keep/cancel、refund/change、预算/风险取舍这类正式决策，仍优先走 knowledge-eval 流程。
