...
For metrics's common workflows, use `skills/knowledge-eval/` before giving formal recommendations when the task is about:

- anomaly confirmation or escalation
- metric definition or reporting interpretation
- attribution or root-cause judgement
- publishing a metrics conclusion with evidence
...
3. Answer in metrics style: conclusion, metric/source boundary, uncertainty, and next validation step

If `summary.status` is `no_hit` or `weak_hit`, say that explicitly and do not overstate precision.
