/**
 * Example: Pre-rendered Components & Chart Beautification
 *
...
async function main() {
  ...
  // Create beautified chart
  const chartData = createBeautifiedChart(data, 'bar', autoPreset);
  slide2.addChart(ChartType.bar, chartData, {
    x: 1, y: 0.5, w: 8, h: 4
  });
  ...
  // ============================================
  // Slide 3: Multiple Chart Presets
  // ============================================
  console.log('📄 Creating Slide 3: Multiple Chart Presets...');
  ...
  for (let i = 0; i < Math.min(presets.length, 5); i++) {
    ...
    // Create small chart for each preset
    const smallData = [
      { name: 'A', labels: ['Value'], values: [10] },
      { name: 'B', labels: ['Value'], values: [20] },
      { name: 'C', labels: ['Value'], values: [15] }
    ];

    const smallChartData = createBeautifiedChart(smallData, 'bar', preset.name);
    slide3.addChart(ChartType.bar, smallChartData, {
      x: 0.5 + (i % 3) * 3,
      y: y + Math.floor(i / 3) * 2.5,
      w: 2.5, h: 2
    });

    // Add preset name
    slide3.addText(preset.displayName, {
      x: 0.5 + (i % 3) * 3,
      y: y + Math.floor(i / 3) * 2.5 + 2.05,
      w: 2.5, h: 0.3,
      fontSize: 10,
      color: '40695B',
      align: 'center'
  ...
  // Add title
  slide3.addText('Available Chart Presets', {
    x: 0.2, y: 0.2, w: 9.6, h: 0.4,
    fontSize: 28,
    color: 'B165FB',
    bold: true
  });
  ...
  console.log('✅ Slide 3: Multiple chart presets comparison');
