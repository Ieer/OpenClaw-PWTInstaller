...
> 版本基线：OpenClaw 2026.4.9
>
> 仓库对齐原则：本仓库统一以 [../openclaw-release.yaml](../openclaw-release.yaml) 中的 `openclaw_version: 2026.4.9` 作为文档基线；当 CLI 外部资料与当前仓库脚本、compose、README 不一致时，以当前仓库实现为准。
...
这份 cheatsheet 按真实使用生命周期组织，目标不是覆盖所有子命令，而是给出一套在 OpenClaw 2026.4.9 上更稳定、便于系统迭代的命令视图。
...
OpenClaw 2026.4.9 延续双层配置思路：全局配置与工作区 / agent 配置并存，局部优先。
...
3. 第三优先级：OpenClaw 2026.4.9 外部参考资料与 `openclaw --help`
